      ...
      if (nextRace != null) {
         ...
         for (com.f1dashboard.entity.RaceSession s : sessions) {
            if (s.getSessionDate() != null && s.getSessionTime() != null) {
               java.time.LocalDateTime sessionDateTime = java.time.LocalDateTime.of(s.getSessionDate(),
                     s.getSessionTime());
         ...
         // Fallback to Race if no specific session is strictly in the future (or no
         // sessions found)
