...
const TimelineNode: React.FC<{ event: TimelineEvent; isLast: boolean }> = ({ event, isLast }) => {
  ...
  return (
    <div className="relative pl-14 pb-6 group">
      {/* Node dot */}
      <div className={`absolute left-4 top-2 w-5 h-5 rounded-full border-2 z-10 transition-all ${
        isCompleted
          ? event.leadChanged
      ...
      {/* Event card */}
      <div className={`glass-card p-5 transition-all duration-300 group-hover:translate-x-1 ${
        !isCompleted ? 'opacity-60 border-dashed' : ''
      }`}>
        {/* Header row */}
