      ...
      <div className="flex gap-4 border-b border-f1-mid-gray/30 pb-0">
        {(['driver', 'constructor'] as const).map(group => (
          <button
            ...
            className={`pb-3 px-1 text-sm font-semibold uppercase tracking-wider transition-colors border-b-2 ${
        ...
        {CATEGORIES.filter(c => c.group === (activeCategory.startsWith('driver') ? 'driver' : 'constructor')).map(cat => {
          ...
          return (
            <button
              ...
              className={`glass-card px-4 py-2 flex items-center gap-2 text-sm font-semibold transition-all ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 0, 2].map((podiumIdx, visualIdx) => {
            ...
            return (
              <div
                key={podiumIdx}
                className={`glass-card p-6 text-center relative overflow-hidden transition-all ${
                  isGold ? 'md:order-2 md:-mt-4 ring-1 ring-yellow-500/30' : isSilver ? 'md:order-1' : 'md:order-3'
                }`}
              >
                {/* Glow */}
                <div
