...
const CornerMarker: React.FC<CornerMarkerProps> = ({ corner, pathId, selected, onHover, onSelect }) => {
   ...
   return (
      ...
      >
         <circle r={selected ? 9 : 6.5} fill={selected ? '#E10600' : '#15151E'} stroke="#F5F5F5" strokeWidth={1.5} className="transition-all duration-200" />
         <text y="2.5" textAnchor="middle" fontSize="7" fontWeight="700" fill="#F5F5F5">{corner.number}</text>
      </g>
   );
};
