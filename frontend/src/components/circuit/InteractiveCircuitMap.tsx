...
import { Activity, Flag, Gauge, Info, MapPin, Ruler, Timer, Zap } from 'lucide-react';
...
type Tooltip =
  ...
  | { kind: 'drs'; title: string; rows: Array<[string, string | number]> }
...
const DetectionPoint: React.FC<{ pathId: string; point: { id: string; label: string; positionPercent: number } }> = ({ pathId, point }) => {
  ...
  return (
    <g transform={`translate(${position.x} ${position.y})`}>
      <rect x="-6" y="-6" width="12" height="12" rx="2" fill="#22c55e" stroke="#dcfce7" strokeWidth="1.5" />
      <text x="11" y="4" fontSize="9" fill="#bbf7d0">{point.label}</text>
    </g>
  );
};
...
const InteractiveCircuitMap: React.FC<InteractiveCircuitMapProps> = ({ circuit }) => {
  ...
  const [hoveredDrs, setHoveredDrs] = useState<CircuitDRSZone | null>(null);
  ...
  const handleDrsHover = (zone: CircuitDRSZone | null) => {
    setHoveredDrs(zone);
    setTooltip(zone && {
      kind: 'drs',
      title: zone.label,
      rows: [['Start', `${zone.startPercent}% lap`], ['End', `${zone.endPercent}% lap`], ['Detection', zone.detectionPointId]],
    });
  };
      ...
      >
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3 px-1">
          ...
          <div className="flex flex-wrap gap-2 text-xs text-f1-silver">
            ...
            <span className="rounded-md border border-green-400/30 bg-green-500/10 px-2 py-1 text-green-200">DRS</span>
          </div>
        </div>
        ...
        <svg viewBox={circuit.viewBox} role="img" aria-label={`${circuit.name} interactive circuit map`} className="h-[420px] w-full sm:h-[560px]">
          ...
          <path d={circuit.trackPath} fill="none" stroke="#020617" strokeWidth="29" strokeLinecap="round" strokeLinejoin="round" />
          <path d={circuit.trackPath} fill="none" stroke="#f8fafc" strokeWidth="16" strokeLinecap="round" strokeLinejoin="round" filter={`url(#${pathId}-glow)`} opacity="0.9" />
          <path d={circuit.trackPath} fill="none" stroke="#111827" strokeWidth="9" strokeLinecap="round" strokeLinejoin="round" />
          ...
          {circuit.drsZonesData.map((zone) => (
            <DRSZone key={zone.id} path={circuit.trackPath} zone={zone} active={hoveredDrs?.id === zone.id} onHover={handleDrsHover} />
          ))}
      ...
      <aside className="space-y-4">
        <div className="rounded-xl border border-white/10 bg-f1-dark-gray/80 p-4 shadow-xl backdrop-blur-md">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Stat icon={<Gauge className="h-4 w-4 text-green-300" />} label="DRS" value={circuit.drsZones} />
